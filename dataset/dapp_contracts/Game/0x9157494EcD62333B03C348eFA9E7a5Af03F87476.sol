...
contract EtherKnightGame
{
    ...
    isMaintenance
    {
        ...
        require( this_gRND>0 && GameRoundData[ this_gRND].extraData[0]>0 && GameRoundData[ this_gRND].extraData[0]0)
        {
    ...
    onlyDirectTransaction
    {
        ...
        require( maintenanceMode==false  && this_gRND>0 && (eth>=minimumSharePrice) && (eth <=100 ether) &&  perso<=3 && action <=5 && block.number 0 );
    ...
    onlyDirectTransaction
    {
        ...
        require( maintenanceMode==false  && this_gRND>0 && (eth>=minimumSharePrice) &&(eth <=100 ether) && perso<=3 && action <=5 && block.number magnitude)
        {
    ...
    function actionApple( uint32 perso , GameVar_s gamevar) pure private
    {
        ...
        gamevar.event_target = 1< 150) gamevar.perso_energy[ perso ] = 150;
