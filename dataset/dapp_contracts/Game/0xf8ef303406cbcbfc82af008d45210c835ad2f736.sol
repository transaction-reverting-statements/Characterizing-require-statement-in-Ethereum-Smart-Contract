...
contract EtherVillains is ERC721 {
  ...
  function pinchVillain(uint256 _victim, uint256 _pincher) public payable returns (bool){
    ...
    require(villains[_pincher].numSkillActive=operationPrice && villains[_victim].state==1 && now< villains[_victim].zappedExipryTime){
        // squeeze
  ...
  function guardVillain(uint256 _target, uint256 _guard) public payable returns (bool){
    ...
    require(villains[_guard].numSkillActive=operationPrice && villains[_target].state<2){
        // guard this villain
