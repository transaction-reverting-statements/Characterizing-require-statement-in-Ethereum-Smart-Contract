...
contract RISK{
    ...
    public
    {
        ...
        require(_roundstart+Rounds[_rID].trucetime0){
                if(t!=team){
                    require(false);
                }
            }
    ...
    returns(bool)
    {
        for (uint i = 0; i 0, "string must be between 1 and 64 characters");
    ...
    returns(bytes32[]){
        ...
        for (uint256 i = 1;iuint16) teamXaddr; //return team by address
